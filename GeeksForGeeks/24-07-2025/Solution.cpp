...
class Solution {
  public:
    int getLastMoment(int n, vector<int>& left, vector<int>& right) {
        ...
        // When two ants collide and reverse directions,
        // the net result is as if they just passed through each other.
        // Since ants are indistinguishable, swapping directions has no effect on the outcome.
        
        int maxTime = 0;

        for(int pos : left) {
            maxTime = max(maxTime, pos); // Time to fall off for left-moving ant
        }
        
        for(int pos : right) {
            maxTime = max(maxTime, n - pos); // Time to fall off for right-moving ant
        }
